...
sub fix_008 {
    ...
    foreach my $f (@flist) {
        ...
        if (length($fdata) == 37) {
            if (substr($fdata, 6, 1) eq 's') {
                $fdata = substr($fdata, 0, 11)."   ".substr($fdata, 11);
                $fixer->msg("CHANGED A:'".$ofdata."'=>'".$fdata."'");
            }
        } elsif (length($fdata) == 36) {
            if (substr($fdata, 6, 1) eq 's' && substr($fdata, 11, 1) !~ /^ $/) {
                $fdata = substr($fdata, 0, 11)."    ".substr($fdata, 11);
                $fixer->msg("CHANGED B:'".$ofdata."'=>'".$fdata."'");
            }
        }

        if (length($fdata) != 40) {
            $fixer->error("008.length != 40 (".length($fdata).":[".$fdata."])");
            next;
        }
	...
	if ($fdata =~ /^([0-9]{6}| {6})(.)(....)(....)(...)(..)(.)(.)(.)(.)(......)(..)(.)(.)(.)(...)(.)(.)$/) {
	    ...
	    my $catsrc = $18;    # 39
            ...
            #}
            $fdata = str_replace_nth($fdata, 24, "||||||") if ($accomp =~ /^\|/);
